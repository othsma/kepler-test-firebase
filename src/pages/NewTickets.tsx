...
export default function NewTickets() {
  ...
  const { tickets, updateTicket, deleteTicket, filterStatus, setFilterStatus, loading, error, fetchTickets, fetchSettings } = useTicketsStore();
          ...
          {isAddingClient ? (
            ...
            <NewTicketForm
              ...
              initialData={editingTicket ? {
                deviceType: tickets.find(t => t.id === editingTicket)?.deviceType || '',
                brand: tickets.find(t => t.id === editingTicket)?.brand || '',
                model: tickets.find(t => t.id === editingTicket)?.model || '',
                tasks: tickets.find(t => t.id === editingTicket)?.tasks || [],
                taskPrices: tickets.find(t => t.id === editingTicket)?.taskPrices,
                issue: tickets.find(t => t.id === editingTicket)?.issue || '',
                cost: tickets.find(t => t.id === editingTicket)?.cost || 0,
                passcode: tickets.find(t => t.id === editingTicket)?.passcode || '',
                status: tickets.find(t => t.id === editingTicket)?.status || 'pending',
                technicianId: tickets.find(t => t.id === editingTicket)?.technicianId
              } : undefined}
            />
          )}
        </div>
